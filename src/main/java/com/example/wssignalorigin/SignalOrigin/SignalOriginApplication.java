...
public class SignalOriginApplication {
	...
	public static void main(String[] args) throws InterruptedException, ExecutionException {
            ...
            String url = "wss://signalprototype.herokuapp.com/signal-in-websocket";
            StompSessionHandler sessionHandler = new MyStompSessionHandler();
            StompSession session = stompClient.connect(url, headers, sessionHandler).get();
            
            session.subscribe("/topic/BloodOxygensignaltoserver" , new StompFrameHandler() {
                @Override
            ...
            session.send("/app/saveBloodOxygenfrequency/789", 250);
            SignalServices signalServices=new SignalServicesImpl1();
            try {
                while(true){
                    Thread.sleep(4);
                    session.send("/app/saveBloodOxygen/789", signalServices.getSignal(Long.parseLong("789")));
                }
